...
class ErrorReportAgent(BaseAgent):
    async def run(self, payload_json: str) -> str:
        prompt = f"""
            ...
            작업:
            1. “summary” 필드에는 에러의 배경과 처리 과정을 아우르는 상세한 한글 문단(3~5문장)으로 작성하세요.
            2. “appliedFiles” 필드에는 fileFixes 배열에서 각 path의 파일명을 추출해 영어 배열 형태로 나열하세요. (경로 제외, 파일명만)
            3. “additionalNotes” 필드에는 fileFixes의 explanation이나, 보고서에 포함할 기타 유의사항을 자유롭게 기술하세요.  
